...
def create_profit_chart(results: List[Dict], top_n: int = 10) -> go.Figure:
    ...
    fig.update_layout(
        title=dict(text="Top Profitable Chains", font=dict(color='#ffd700', size=16)),
        xaxis=dict(title="Net Profit (GP)", title_font=dict(color='#f4e4bc', size=11),
                   tickfont=dict(color='#f4e4bc', size=9), gridcolor='rgba(139,115,85,0.25)', tickformat=',.0f'),
        yaxis=dict(title="", tickfont=dict(color='#f4e4bc', size=9), autorange="reversed"),
...
def create_category_pie(results: List[Dict]) -> go.Figure:
    ...
    fig = go.Figure(data=[
        go.Pie(
            ...
            textfont=dict(color='#f4e4bc', size=10),
            marker=dict(colors=colors, line=dict(color='#1a2a3a', width=2)),
            pull=[0.03 if i == 0 else 0 for i in range(len(labels))],
            sort=False
        )
    ])
    
    fig.add_annotation(text=f"<b>Total</b><br>{format_gp(total_profit)}", x=0.5, y=0.5,
                       font=dict(color='#ffd700', size=12), showarrow=False)
    
    fig.update_layout(
        title=dict(text="Profit by Category", font=dict(color='#ffd700', size=16)),
        height=400, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(26,42,58,0.8)',
        showlegend=False, margin=dict(l=40, r=40, t=50, b=40)
    )
...
def create_profit_histogram(profits: List[float], per_item: bool = False) -> go.Figure:
    ...
    q1, q3 = np.percentile(profits_arr, 25), np.percentile(profits_arr, 75)
    iqr = q3 - q1
    ...
    hist_data = main_data if has_extreme_outliers and len(main_data) > 0 else profits_arr
    
    profitable = hist_data[hist_data > 0]
    unprofitable = hist_data[hist_data <= 0]
    
    if len(hist_data) > 1:
        hist_range = hist_data.max() - hist_data.min()
        bin_size = hist_range / 20 if hist_range > 0 else 1000
        if bin_size > 0:
    ...
    if len(profitable) > 0:
        fig.add_trace(go.Histogram(
            x=profitable, name=f'Profitable ({len(profitable)})',
            marker_color=CHART_COLORS['gold'], marker_line_color=CHART_COLORS['gold_dark'],
            marker_line_width=1, opacity=0.9, xbins=dict(size=bin_size)
        ))
    
    if len(unprofitable) > 0:
        fig.add_trace(go.Histogram(
            x=unprofitable, name=f'Unprofitable ({len(unprofitable)})',
            marker_color=CHART_COLORS['dragon_red'], marker_line_color=CHART_COLORS['dragon_red_dark'],
            marker_line_width=1, opacity=0.9, xbins=dict(size=bin_size)
        ))
    
    fig.add_vline(x=0, line_dash="solid", line_color="rgba(244,228,188,0.5)", line_width=2)
    fig.add_vline(x=median_val, line_dash="dot", line_color=CHART_COLORS['rune_blue'], line_width=2)
    
    fig.update_layout(
        title=dict(text="Profit Distribution", font=dict(color='#ffd700', size=16)),
        xaxis=dict(title=f"Net Profit ({unit_label})", title_font=dict(color='#f4e4bc', size=11),
                   tickfont=dict(color='#f4e4bc', size=9), gridcolor='rgba(139,115,85,0.25)'),
        yaxis=dict(title="Count", title_font=dict(color='#f4e4bc', size=11),
                   tickfont=dict(color='#f4e4bc', size=9), gridcolor='rgba(139,115,85,0.25)'),
        height=350, margin=dict(l=55, r=20, t=60, b=50),
        paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(26,42,58,0.8)',
        bargap=0.05, barmode='overlay',
        legend=dict(font=dict(color='#f4e4bc', size=10), bgcolor='rgba(26,42,58,0.8)',
...
def create_roi_scatter(results: List[Dict]) -> Optional[go.Figure]:
    ...
    for cat, cat_results in categories.items():
        ...
        fig.add_trace(go.Scatter(
            x=profits, y=rois, mode='markers', name=cat,
            marker=dict(size=10, color=color, line=dict(width=1, color='#1a2a3a')),
            text=items,
    ...
    fig.update_layout(
        title=dict(text="ROI vs Profit Analysis", font=dict(color='#ffd700', size=16)),
        xaxis=dict(title="Net Profit (GP)", title_font=dict(color='#f4e4bc', size=11),
                   tickfont=dict(color='#f4e4bc', size=9), gridcolor='rgba(139,115,85,0.25)'),
        yaxis=dict(title="ROI (%)", title_font=dict(color='#f4e4bc', size=11),
                   tickfont=dict(color='#f4e4bc', size=9), gridcolor='rgba(139,115,85,0.25)'),
        height=400, margin=dict(l=55, r=20, t=60, b=80),
        paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(26,42,58,0.8)',
        legend=dict(font=dict(color='#f4e4bc', size=10), bgcolor='rgba(26,42,58,0.8)',
                    orientation='h', yanchor='bottom', y=-0.25, xanchor='center', x=0.5)
    )
...
def create_category_comparison(results: List[Dict]) -> go.Figure:
    ...
    categories, bests, medians, averages = [], [], [], []
    ...
    fig.add_trace(go.Bar(name='Best', x=categories, y=bests, marker_color=CHART_COLORS['gold'],
                         text=[format_gp(v) for v in bests], textposition='outside',
                         textfont=dict(size=9, color='#f4e4bc')))
    fig.add_trace(go.Bar(name='Median', x=categories, y=medians, marker_color=CHART_COLORS['rune_blue'],
                         text=[format_gp(v) for v in medians], textposition='outside',
                         textfont=dict(size=9, color='#f4e4bc')))
    fig.add_trace(go.Bar(name='Average', x=categories, y=averages, marker_color=CHART_COLORS['driftwood'],
                         text=[format_gp(v) for v in averages], textposition='outside',
                         textfont=dict(size=9, color='#f4e4bc')))
    
    fig.update_layout(
        title=dict(text="Category Comparison", font=dict(color='#ffd700', size=16)),
        xaxis=dict(title="", tickfont=dict(color='#f4e4bc', size=9), tickangle=45),
        yaxis=dict(title="Profit (GP)", title_font=dict(color='#f4e4bc', size=11),
                   tickfont=dict(color='#f4e4bc', size=9), gridcolor='rgba(139,115,85,0.25)'),
        height=400, margin=dict(l=55, r=20, t=60, b=100),
        paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(26,42,58,0.8)',
        barmode='group',
        legend=dict(font=dict(color='#f4e4bc', size=10), bgcolor='rgba(26,42,58,0.8)',
